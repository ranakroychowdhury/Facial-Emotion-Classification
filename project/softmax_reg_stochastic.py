# -*- coding: utf-8 -*-
"""
Created on Mon Jan 13 21:38:36 2020
...
def sigmoid(data, weight):
    ...
    for i in range(n):
        activation[i] = activation[i] / summation[i]
    return activation
...
def softmax_reg_stochastic(train_data, val_data, test_data, train_label, val_label, test_label, epoch, classes, learning_rate):
    ...
    for t in range(1, epoch + 1):
        ...
        np.random.shuffle(P)
        for i in range(n):
            error = train_label[P[i]] - sigmoid_result[P[i]]
            error = error.reshape(classes, 1)
            obs = np.copy(train_data[P[i]])
            obs = obs.reshape(1, d)
            weight += learning_rate * np.matmul(error, obs)
...
if __name__ == '__main__':
    ...
    training_error, validation_error, validation_acc, test_error, test_acc, TEST_ERROR, TEST_ACCURACY = softmax_reg_stochastic(train_data, val_data, test_data, train_label, val_label, test_label, epoch, classes, learning_rate)
